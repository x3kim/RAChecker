...
export function GenrePanel() {
  ...
  // Two effects on purpose: the scope chip re-reads the facets, but must not
  // drag the stream cleanup along — that would close a running job's
  // EventSource while esRef stays set, and start() refuses to run again.
  useEffect(() => { load(); /* eslint-disable-next-line */ }, [ownedOnly]);
