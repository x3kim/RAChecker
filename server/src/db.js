...
// rebuilds those rows and would otherwise drop the genre. Rows stored as "asked,
// RA has no genre" are skipped: writing NULL over NULL changes nothing but would
// be counted as restored, and every boot would claim work it never did.
export function restoreGameGenres() {
  const info = db.prepare(`
    UPDATE games SET genre = (SELECT gg.genre FROM game_genres gg WHERE gg.game_id = games.id)
    WHERE genre IS NULL
      AND EXISTS (SELECT 1 FROM game_genres gg WHERE gg.game_id = games.id AND gg.genre IS NOT NULL)
  `).run();
  backfillGenreMajor();
  return Number(info.changes ?? 0);
}
